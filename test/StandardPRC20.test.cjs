...
describe("StandardPRC20", function () {
    ...
    const MAX_WALLET_AMOUNT = ethers.parseEther("20000");
    ...
    describe("Deployment", function () {
        ...
        it("Should exclude owner from transfer limits", async function () {
            ...
            expect(await token.isExcludedFromTransferLimits(await token.getAddress())).to.be.true;
        });
    ...
    describe("Transfer Limits", function () {
        ...
        it("Should enforce max transaction amount", async function () {
            const overLimit = MAX_TX_AMOUNT + 1n;
        ...
        it("Should enforce max wallet amount", async function () {
            // Try to transfer amount that would exceed wallet limit but not tx limit
            // addr2 starts with 0 balance, max wallet is 20000, so sending 9000 twice should fail on second if total > 20000
            await token.connect(addr1).transfer(addr2.address, ethers.parseEther("9000"));
            // Now try to send another 9000 to make total 18000 (still under limit)
            await token.connect(addr1).transfer(addr2.address, ethers.parseEther("9000"));
            // Now try to send more to exceed wallet limit (total would be 18000 + 5000 = 23000 > 20000)
            await expect(token.connect(addr1).transfer(addr2.address, ethers.parseEther("5000")))
    ...
    describe("Minting", function () {
        ...
        it("Should not allow non-owner to mint", async function () {
            await expect(token.connect(addr1).mint(addr2.address, ethers.parseEther("1000")))
                .to.be.revertedWithCustomError(token, "OwnableUnauthorizedAccount");
        });
        ...
        it("Should not allow minting to zero address", async function () {
            await expect(token.mint(ethers.ZeroAddress, ethers.parseEther("1000")))
    ...
    describe("Emergency Functions", function () {
        ...
        it("Should not allow non-owner to emergency withdraw", async function () {
            await expect(token.connect(addr1).emergencyWithdraw(ethers.ZeroAddress, 0))
                .to.be.revertedWithCustomError(token, "OwnableUnauthorizedAccount");
        });
    });
    ...
    describe("Edge Cases", function () {
        ...
        it("Should reject invalid constructor parameters", async function () {
            ...
            await expect(StandardPRC20.deploy(
                ...
                MAX_TX_AMOUNT,
                MAX_WALLET_AMOUNT
            )).to.be.revertedWithCustomError(StandardPRC20, "OwnableInvalidOwner");
        });
    });
});